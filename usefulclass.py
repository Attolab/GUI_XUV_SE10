...
class PeakFitter():
    ...
    def make_initial_guess(x, y, peak_indices =None, results_half=None, prominence = 0.5):
            peak_indices, results_half = PeakFinder.find_peaks_scipy(y, prominence=prominence, rel_height=0.5)
    ...
    def n_gaussian_fit(y, x=np.zeros((1,0)),prominence = 0.5):
        if not(x.size):        
            x = np.arange(len(y))
        number_of_peaks, initial_guess, lb, ub = PeakFitter.make_initial_guess(x, y, prominence = prominence)
...
class PeakFinder():
    # Basic peak finder
    def find_peaks_scipy(y, prominence=0.5, rel_height=0.5):
        peak_indices, _ = sc_sig.find_peaks(y, prominence=prominence,)
